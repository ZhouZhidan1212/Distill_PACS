...
class MultipleDomainDataset:
    ...
    def __getitem__(self, index):
        ...
        """
        return self.datasets[index] # type: ignore

    def __len__(self):
        """
        Return: # of sub-datasets
        """
        return len(self.datasets) # type: ignore
...
class RotatedMNIST(MultipleEnvironmentMNIST):
    ...
    def rotate_dataset(self, images, labels, angle):
        rotation = T.Compose(
            [
                T.ToPILImage(),
                T.Lambda(lambda x: rotate(x, angle, fill=(0,), resample=Image.BICUBIC)), # type: ignore
                T.ToTensor(),
            ]
        )
...
class MultipleEnvironmentImageFolder(MultipleDomainDataset):
    def __init__(self, root):
        ...
        #breakpoint()
        self.test_env = environments.pop() 
        environments = sorted(environments)
        environments.append(self.test_env)
        self.environments = environments
        #breakpoint()
...
class PACS(MultipleEnvironmentImageFolder):
    ...
    def __init__(self, root):
        self.dir = os.path.join(root, "/home/zhidan/zhidan/PACS/")
        super().__init__(self.dir)
...
if __name__=='__main__':
    images = PACS('/home/zhidan/zhidan/')
    print(images)
    print(len(images))